...
struct alignment{
	vector<int> path;
	vector<int> scores;
	uint position_anchors_in_read;
	uint position_anchors_in_unitig;
	uint unitig_number_anchored;
	int score;
	//~ uint weight;
};
...
public:
	...
	vector<uint16_t> unitigs_weight;
	...
	void alignment_clean(alignment& al);
	string get_corrected_read(const alignment& al, const string& read);
	string alignReadOpti_correction(const string& read, alignment& al);
